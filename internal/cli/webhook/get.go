...
func runGet(cmd *cobra.Command, args []string) error {
	...
	// Get webhook
	webhook, err := client.Admin().GetWebhook(ctx, webhookID)
	if err != nil {
...
func printWebhookDetails(wh *vaultsandbox.Webhook) {
	...
	if wh.Filter != nil && (len(wh.Filter.Rules) > 0 || wh.Filter.RequireAuth) {
		...
		fmt.Printf("  %s %s\n", labelStyle.Render("Mode:"), fmt.Sprintf("%s (%s)", wh.Filter.Mode, filterModeDescription(wh.Filter.Mode)))
...
func filterModeDescription(mode vaultsandbox.FilterMode) string {
	switch mode {
