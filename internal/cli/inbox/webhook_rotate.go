...
you to update your endpoint without downtime.`,
	...
	RunE: runWebhookRotate,
}

var whRotateForce bool

func init() {
	webhookCmd.AddCommand(webhookRotateCmd)

	webhookRotateCmd.Flags().BoolVarP(&whRotateForce, "force", "f", false,
		"Skip confirmation prompt")
}

func runWebhookRotate(cmd *cobra.Command, args []string) error {
	ctx := context.Background()
	jsonMode := cliutil.GetOutput(cmd) == "json"
	webhookID := args[0]

	// Confirmation
	if !whRotateForce && !jsonMode {
	...
	// Rotate secret
	resp, err := inbox.RotateWebhookSecret(ctx, webhookID)
	if err != nil {
	...
	// Pretty output
	printInboxRotateResult(resp)
	return nil
}

func printInboxRotateResult(resp *vaultsandbox.RotateSecretResponse) {
