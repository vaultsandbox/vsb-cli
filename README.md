...
<!-- TODO: Add hero GIF showing the TUI dashboard -->
...
vsb config set api_key "your-api-key"
vsb config set url "https://your-gateway.vsx.email"
...
<!-- TODO: Add GIF showing inbox creation and TUI launch -->
![Quick Start](./assets/demo-quickstart.gif)

## TUI Dashboard

Run `vsb` without arguments to launch the interactive dashboard. It watches all your stored inboxes in real-time.

<!-- TODO: Add GIF showing TUI navigation between tabs -->
...
<!-- TODO: Add GIF showing inbox commands -->
![Inbox Management](./assets/demo-inbox.gif)
...
vsb wait --timeout 30s

# Wait for email with specific subject
vsb wait --subject "Password Reset"

# Wait for email matching regex
vsb wait --subject-regex "Verify.*email"

# Wait for email from specific sender
vsb wait --from "noreply@example.com"

# Output email as JSON for scripting
vsb wait --json | jq '.links[0]'
```
...
- name: Test password reset email
  run: |
    ...
    RESET_LINK=$(vsb wait --subject-regex "Reset" --json | jq -r '.links[0]')
...
vsb export <email-address> -o inbox-backup.json
...
2. **Environment variables** — `VSB_API_KEY`, `VSB_URL`
...
url: https://your-gateway.vsx.email
```
